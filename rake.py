...
class Rake(object):
    ...
    def build_stop_word_regex(self, extra = []):
        stop_word_list = self.load_stop_words()
        stop_word_list.extend(extra)
    ...
    def generate_candidate_keyword_scores(self, phrase_list, word_score, min_score = 0):
        keyword_candidates = []
        for phrase in phrase_list:
    ...
    def run(self, sentences, stop_words = []):
        stop_words_pattern = self.build_stop_word_regex(stop_words)
        phrase_list = self.generate_candidate_keywords(sentences, stop_words_pattern)
        ...
        keyword_candidates = sorted(keyword_candidates, key=lambda e: e['score'], reverse=True)
